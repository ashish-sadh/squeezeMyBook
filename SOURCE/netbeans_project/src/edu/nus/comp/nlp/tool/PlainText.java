 ...
 *
 * History:
 * 2010.01.03 Bug fixing: In splitSentences() the size of <i>sents</i> wasn't checked before trying to remove something
 *            from it, at one point. Corrected. ( Thanks to Chisom. ) 
 * 2006.11.01 V2.04.1 Unpaired double-quotation mark in the input text: If there is one at the then of the input text and
 * for some reason (ill-formated input, for instance) it is not recognized as a closing one, It'll form a pseudo sentence
 * with only a "\"", which won't be parsed, syntactically or semantically. Such sentences without parses cause problem in
 * downstream analysiers. Solution: Attach it to the last sentence.
 * 2006.11.01 V2.04 Bug fixing: Ending sentence like "A is U.S." will no longer be added twice, either.
...
public class PlainText implements java.io.Serializable{
  ...
  static public String[] splitSentences(String text, String delim) {
    ...
    String delimForTokenizer = "!|\"|\\.|\\?"; //| is not necessary here, added for the sake of checking "tmp.matches(delimForTokenizer"
          ...
          continue;
            ...
            if (acronym || (title_acronym.length()>0) ) {
              ...
              if (!tokenizer.hasMoreTokens()) {
                ...
                title_acronym = ""; //Reset: The content till the very end of the input has been fit into a sentence. Nov.1 2006
              }
            }else if (tmp.equals("\"") && openQuotation
                    && tokenizer.hasMoreTokens()) { //Nov.1, 2006. See @History 2006.11.01 V2.04.1
              ...
              if (sents.size() > 0) { //This wasn't checked until 2010.01.03, as suggested by Chisom.
            	  sents.add( ( (String) sents.remove(sents.size() - 1)) +
            			  tmp + " ");
              }
              else {
            	  //Happens when the article starts with an opening quot
            	  sents.add(tmp + " ");
              }
            }
            continue;
  ...
  public static void main (String[] args){
      ...
      if(args.length==0 || mode.length()==0){
          showHelpMessage();
          System.exit(0);
      }
      ...
      if(mode.equals("getPlainText")){
          getPlainText(inputFile);
      }
  ...
  public static void getPlainText(String fileName){
      PlainText pText = new PlainText(fileName);
      pText.removeTag();
      System.out.println(pText.text);
  }

  private static void showHelpMessage(){
      PlainText dummy = new PlainText();
      System.err.println("Usage:");
      System.err.println(dummy.getClass()+" [options] inputFile");
      System.err.println("\t-p\tget plain text");
  }
